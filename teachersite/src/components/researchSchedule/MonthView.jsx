...
export default function MonthView({
  ...
  const semesterMonthStart = selectedSemester
    ? startOfMonth(new Date(selectedSemester.start))
    : null;
  const semesterMonthEnd = selectedSemester
    ? startOfMonth(new Date(selectedSemester.end))
    : null;
    ...
    Object.keys(map).forEach((key) => {
      map[key].sort((a, b) => {
        const aSlot =
          a.time_slot_id ??
          a.time_slot_idx ??
          (a.time_slot && a.time_slot.id) ??
          null;
        const bSlot =
          b.time_slot_id ??
          b.time_slot_idx ??
          (b.time_slot && b.time_slot.id) ??
          null;
  ...
  return (
    <div className="month-view card p-5 rounded-xl shadow-md bg-white">
      ...
      <div
        ...
        style={{ background: "linear-gradient(90deg,#4A90E2 0%,#5BA3F5 100%)" }}
      >
      ...
      <div
        className="mb-4 bg-[#f8f9fa] border-b"
          ...
          <React.Fragment key={wi}>
            {week.map((day) => {
              ...
              const todayClass = isToday
                ? "bg-[#fffbf0] border-yellow-200 ring-1 ring-yellow-100"
                : "";
                    ...
                    {dayEvents.map((ev, idx) => {
                      ...
                      return (
                        <div
                          ...
                          style={{
                            background:
                              "linear-gradient(180deg,#eefcf1 0%,#e1f7e8 100%)",
                          }}
                          title={ev.title}
                        >
                          {ev.title}
