          ...
          loadingDetail ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                <button
                  onClick={() => { if (!isPastDate) setOpenExternalEditModal(true); }}
                  disabled={isPastDate}
                  title={isPastDate ? 'Không thể chỉnh sửa buổi học trong quá khứ' : 'Chỉnh sửa buổi học'}
                  className={`px-4 py-2 text-sm font-medium text-white bg-amber-600 hover:bg-amber-700 rounded-lg transition-colors flex items-center gap-2 ${isPastDate ? 'opacity-50 cursor-not-allowed hover:bg-amber-600' : ''}`}
                >
                ...
                <ExternalEditScheduleModal
                  open={openExternalEditModal}
                  onClose={() => setOpenExternalEditModal(false)}
                  onSave={(payload) => {
                    // Merge payload with existing detail and forward to parent handler
                    try {
                      onEdit({ ...(fullDetail || detail), ...payload });
                    } finally {
                      setOpenExternalEditModal(false);
                    }
                  }}
