  ...
  const handleSave = async () => {
    if (!validateEmail(editEmail)) {
      ...
      try {
        ...
        // use apiClient directly for update (keeps existing interceptor/headers)
        const res = await authService.apiClient.put('/teachersite/profile', payload);
        const updated = res?.data?.data || res?.data || { ...profile, ...payload };
        setProfile(updated);
            ...
            <div className="mt-4 border border-blue-200 rounded-lg p-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x sm:divide-blue-100">
                ...
                <div className="py-4 px-4">
                  <div className="text-xs text-gray-500">Mã GV</div>
                  <div className="font-medium">{profile?.teacher_identifier || '—'}</div>

                  <div className="text-xs text-gray-500 mt-3">Tên giảng viên</div>
              ...
              <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm text-gray-700">
                <div>
                  ...
                  <div className="font-medium">{profile?.faculty?.name || '—'}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500">Chức danh</div>
                  <div className="font-medium">{profile?.academic_title || '—'}</div>
                </div>
                {/* Removed duplicate employee code display to avoid redundant fields */}
              </div>
