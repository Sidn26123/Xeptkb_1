        ...
        <table className="min-w-full border-collapse border border-blue-200 text-xs">
          <thead>
            <tr className="bg-blue-500 text-white text-xs">
              <th className="p-1 border border-blue-200">Mã MH</th>
              <th className="p-1 border border-blue-200">Tên môn học</th>
              <th className="p-1 border border-blue-200">Nhóm tơ</th>
              <th className="p-1 border border-blue-200">Số tín chỉ</th>
              <th className="p-1 border border-blue-200">Lớp</th>
              <th className="p-1 border border-blue-200">Thứ</th>
              <th className="p-1 border border-blue-200">Tiết bắt đầu</th>
              <th className="p-1 border border-blue-200">Số tiết</th>
              <th className="p-1 border border-blue-200">Phòng</th>
              <th className="p-1 border border-blue-200">Giảng viên</th>
              <th className="p-1 border border-blue-200">Thời gian học</th>
            </tr>
          </thead>
          <tbody>
            {rows.length === 0 && (
              <tr>
                <td className="p-2 text-center" colSpan={11}>Không có dữ liệu</td>
              </tr>
            )}
            {rows.map((r, idx) => (
              <tr key={r.code + idx} className="text-xs hover:bg-gray-50">
                <td className="p-1 border border-blue-200 align-top">{r.code}</td>
                <td className="p-1 border border-blue-200 align-top">{r.title}</td>
                <td className="p-1 border border-blue-200 align-top">{r.group}</td>
                <td className="p-1 border border-blue-200 align-top text-center">{r.credits}</td>
                <td className="p-1 border border-blue-200 align-top">{r.classList}</td>
                <td className="p-1 border border-blue-200 align-top">{r.day}</td>
                <td className="p-1 border border-blue-200 align-top text-center">{r.startTime}</td>
                <td className="p-1 border border-blue-200 align-top text-center">{r.periods}</td>
                <td className="p-1 border border-blue-200 align-top">{r.room}</td>
                <td className="p-1 border border-blue-200 align-top">{r.teacher}</td>
                <td className="p-1 border border-blue-200 align-top">{r.semesterRange}</td>
              </tr>
            ))}
          </tbody>
        </table>
