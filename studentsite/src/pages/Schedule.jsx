...
export default function Schedule() {
  ...
  const mockStudent = useMemo(
    () => ({ id: 1, class_id: 101, name: "Nguyễn Văn A" }),
    []
  );
  ...
  // In a real app we'd fetch classes and student info from the API.
  // For now we keep mockStudent and sample events in-memory.
  ...
  const classSchedules = useMemo(() => {
    return getSampleEvents().filter(
      (s) => String(s.course_class_id) === String(mockStudent.class_id)
    );
  }, [mockStudent.class_id]);
  ...
  // initialize view to student's class schedule by default
  useEffect(() => {
    setEvents(classSchedules);
    setHasSearched(true);
    setSelectedClassId(mockStudent.class_id);
    setFilter({ selectedClassId: mockStudent.class_id, mode });
  }, [classSchedules, mockStudent.class_id, mode]);
