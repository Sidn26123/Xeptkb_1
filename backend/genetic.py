...
class CourseSchedule:
    ...
    def conflicts_with(self, other: 'CourseSchedule') -> Tuple[bool, bool, bool]:
        ...
        class_conflict = not set(self.class_ids).isdisjoint(other.class_ids) #change
...
class TheoryBeforeLabConstraint(BaseConstraint):
    ...
    def calculate_penalty(self, schedule: SemesterSchedule, data: SchedulerData) -> float:
        ...
        course_map = {c.id: c for c in schedule.courses}
        for course in schedule.courses:
    ...
    def calculate_penalty(self, schedule: SemesterSchedule, data: SchedulerData) -> float:
        ...
        for week in range(data.semester_start, data.semester_end + 1):
            ...
            for c in concurrent_courses:
                # Duyệt qua từng lớp con trong mảng class_ids
                for specific_class_id in c.class_ids:
                    schedules_by_class[specific_class_id].append(c)
        ...
        for week in range(data.semester_start, data.semester_end + 1):
            ...
            for course in concurrent_courses:
                for class_id in course.class_ids:
                    schedules_by_class[class_id].append(course)
...
class LimitContinuousPeriodsConstraint(BaseConstraint):
    ...
    def calculate_penalty(self, schedule: SemesterSchedule, data: SchedulerData) -> float:
        ...
        for week in range(data.semester_start, data.semester_end + 1):
            ...
            schedules_by_class = defaultdict(list)
            for c in concurrent_courses:
                for class_id in c.class_ids:
                    schedules_by_class[class_id].append(c)
...
class SemesterGeneticScheduler:
    ...
    def __init__(self,
        ...
        for course_info in self.courses_info:
            ...
            if not eligible_rooms:
                ...
                print(f"  - Course {course_info['id']} eligible rooms: {eligible_rooms}")
    ...
    def _apply_random_mutation_to_gene(self, course: CourseSchedule):
        ...
        if mutation_type == 'shift_weeks':
            ...
            course.room_id = random.choice(self.eligible_rooms_map[course.id])
    ...
    def _create_random_gene(self, course_info: dict) -> CourseSchedule:
        ...
        """
        course_class_id = course_info['id']
        class_ids = course_info.get('class_ids', [])
        ...
        print("Selecting room for course", course_class_id)
        room_id = random.choice(self.eligible_rooms_map[course_class_id])
        ...
        course_schedule = CourseSchedule(
            ...
            id = course_info['id'],
            class_ids=class_ids, # ID lớp học phần
    ...
    def evolve(self, progress_callback=None):
        ...
        for generation in range(self.generations):
            ...
            # [MỚI] GỬI BÁO CÁO TIẾN ĐỘ
            # Chỉ gửi mỗi 5 hoặc 10 thế hệ để đỡ lag, hoặc gửi ở thế hệ cuối cùng
            if progress_callback and (generation % 5 == 0 or generation == self.generations - 1):

                # Tính phần trăm hoàn thành (ước lượng)
                percent_complete = ((generation + 1) / self.generations) * 100

                # Tạo object data báo cáo
                progress_data = {
                    'generation': generation + 1,
                    'max_generations': self.generations,
                    'percent': round(percent_complete, 1),
                    'best_fitness': best_fitness_in_gen,
                    'is_valid': best_fitness_in_gen < self.hard_penalty_threshold,
                    'phase': 'evolution' # Có thể đổi thành 'structure', 'refinement' tùy giai đoạn
                }

                # Gọi hàm callback
                progress_callback(progress_data)

            # [Logic dừng sớm giữ nguyên...]
            if best_fitness_in_gen <= self.target_fitness:
                # Trước khi break cũng nên báo cáo 100%
                if progress_callback:
                    progress_callback({'percent': 100, 'status': 'completed', 'best_fitness': best_fitness_in_gen})
                break
        # Final result
...
if __name__ == "__main__":
    ...
    if valid_schedule: # Chỉ chạy GĐ 2 nếu GĐ 1 thành công
        ...
        if result['best_schedule']:
            ...
            for course in sorted(schedule.courses, key=lambda c: (c.start_week, c.id)):
                # ... (in chi tiết) ...
                print(f"\n   ClassID {course.id} - Course {course.id}:")
