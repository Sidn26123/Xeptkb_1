...
import { AlertTriangle, CheckCircle, XCircle, Activity, Server, Users } from 'lucide-react';

const PreSchedulingDashboard = ({ data, onClose }) => {
    const { courses, rooms, teachers, semester_config } = data;

    // --- 1. CALCULATIONS ---

    // 1.1 Overall Load Factor
    const loadStats = useMemo(() => {
        const totalWeeks = semester_config.end_week - semester_config.start_week + 1;
        // Giả sử 1 tuần có 6 ngày * 12 tiết = 72 slots (hoặc lấy từ config)
        const slotsPerWeek = 60;
        const totalRoomCapacitySlots = rooms.length * slotsPerWeek * totalWeeks;

        const totalRequiredSlots = courses.reduce((acc, c) => {
            return acc + (c.weeks_needed * c.sessions_per_week * c.duration_per_session);
        }, 0);

        const percentage = totalRoomCapacitySlots > 0
            ? ((totalRequiredSlots / totalRoomCapacitySlots) * 100).toFixed(1)
            : 0;

        return { required: totalRequiredSlots, capacity: totalRoomCapacitySlots, percentage };
    }, [courses, rooms, semester_config]);

    // 1.2 Teacher Availability Ratio
    const overloadedTeachers = useMemo(() => {
        const totalWeeks = semester_config.end_week - semester_config.start_week + 1;
        const slotsPerWeek = 72;

        const teacherLoad = {};
        courses.forEach(c => {
            teacherLoad[c.teacher_id] = (teacherLoad[c.teacher_id] || 0) +
                (c.weeks_needed * c.sessions_per_week * c.duration_per_session);
        });
        ...
        Object.keys(teacherLoad).forEach(tid => {
            ...
            if (teacher) {
                const busyCount = (teacher.busy_slots?.length || 0) * totalWeeks; // Ước lượng busy cả kỳ
                const available = (slotsPerWeek * totalWeeks) - busyCount;
                if (teacherLoad[tid] > available) {
                    overloads.push({
                        name: teacher.name,
                        load: teacherLoad[tid],
                        available
                    });
                }
            }
        ...
        return overloads;
    }, [courses, teachers, semester_config]);

    // 1.3 Room Capacity Fit & Size Distribution
    const sizeStats = useMemo(() => {
        const distribution = [
            { name: '< 35', count: 0, roomCount: rooms.filter(r => r.capacity < 35).length },
            { name: '35 - 50', count: 0, roomCount: rooms.filter(r => r.capacity >= 35 && r.capacity <= 50).length },
            { name: '50 - 70', count: 0, roomCount: rooms.filter(r => r.capacity > 50 && r.capacity <= 70).length },
            { name: '> 70', count: 0, roomCount: rooms.filter(r => r.capacity > 70).length },
        ];

        const overflowCourses = [];
        const maxRoomCap = Math.max(...rooms.map(r => r.capacity), 0);

        courses.forEach(c => {
            if (c.student_count < 35) distribution[0].count++;
            else if (c.student_count <= 50) distribution[1].count++;
            else if (c.student_count <= 70) distribution[2].count++;
            else distribution[3].count++;

            if (c.student_count > maxRoomCap) {
                overflowCourses.push(c);
            }
        });

        return { distribution, overflowCourses };
    }, [courses, rooms]);

    // 1.4 Equipment Bottleneck (Giả lập vì input chưa có equipment list chi tiết)
    const equipmentStats = useMemo(() => {
        ...
        courses.forEach(c => {
            c.required_equipment_ids?.forEach(eid => {
                demand[eid] = (demand[eid] || 0) + 1;
            });
        });

        rooms.forEach(r => {
            r.equipment_ids?.forEach(eid => {
                supply[eid] = (supply[eid] || 0) + 1;
            });
        });

        // Convert to array for chart
        // Giả sử ta có list equipment name, ở đây dùng ID tạm
        const allIds = new Set([...Object.keys(demand), ...Object.keys(supply)]);
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-y-auto">
                <div className="p-6 border-b border-gray-200 flex justify-between items-center sticky top-0 bg-white z-10">
                    <div>
                        ...
                        <p className="text-sm text-gray-500">Phân tích khả thi trước khi chạy thuật toán</p>
                    </div>
                ...
                <div className="p-6 space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="bg-gray-50 p-5 rounded-lg border border-gray-200">
                            <h3 className="font-semibold text-gray-700 mb-2 flex justify-between">
                                Tổng quan Tải (Load Factor)
                                <span className={`px-2 py-0.5 rounded text-white text-xs ${getLoadColor(loadStats.percentage)}`}>
                                    {loadStats.percentage}%
                                </span>
                            </h3>
                            <div className="w-full bg-gray-200 rounded-full h-4 mb-2">
                                <div
                                    className={`h-4 rounded-full transition-all duration-500 ${getLoadColor(loadStats.percentage)}`}
                                    style={{ width: `${Math.min(loadStats.percentage, 100)}%` }}
                                ></div>
                            </div>
                            <p className="text-xs text-gray-500 mt-2">
                                {loadStats.required.toLocaleString()} tiết cần xếp / {loadStats.capacity.toLocaleString()} tiết khả dụng.
                                {loadStats.percentage > 85 && <span className="text-red-500 block font-bold mt-1">⚠️ Hệ thống quá tải, khó tìm lời giải!</span>}
                            </p>
                        </div>
                        ...
                        <div className="bg-gray-50 p-5 rounded-lg border border-gray-200">
                            <h3 className="font-semibold text-gray-700 mb-2 flex items-center gap-2">
                                <Server size={18}/> Sức chứa Phòng
                            </h3>
                            {sizeStats.overflowCourses.length > 0 ? (
                                <div className="text-red-600">
                                    <div className="text-3xl font-bold">{sizeStats.overflowCourses.length}</div>
                                    <p className="text-sm">Lớp vượt quá sức chứa phòng lớn nhất</p>
                                </div>
                            ) : (
                                <div className="text-green-600 flex flex-col justify-center h-full">
                                    <div className="flex items-center gap-2 text-xl font-bold"><CheckCircle/> An toàn</div>
                                    <p className="text-xs text-gray-500">Mọi lớp đều có phòng chứa đủ</p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="bg-gray-50 p-5 rounded-lg border border-gray-200">
                            <h3 className="font-semibold text-gray-700 mb-2 flex items-center gap-2">
                                <Users size={18}/> Giảng viên Quá tải
                            </h3>
                            {overloadedTeachers.length > 0 ? (
                                <div className="text-red-600">
                                    <div className="text-3xl font-bold">{overloadedTeachers.length}</div>
                                    <p className="text-sm">Giảng viên không đủ giờ dạy</p>
                                    <div className="mt-2 max-h-20 overflow-y-auto text-xs text-gray-600 bg-white p-2 rounded border">
                                        {overloadedTeachers.map(t => (
                                            <div key={t.name}>{t.name}: Cần {t.load} / Có {t.available}</div>
                                        ))}
                                    </div>
                                </div>
                            ) : (
                                <div className="text-green-600 flex flex-col justify-center h-full">
                                    <div className="flex items-center gap-2 text-xl font-bold"><CheckCircle/> Đủ nhân lực</div>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                            <h3 className="font-bold text-gray-700 mb-4">Phân bố Lớp vs. Phòng theo Quy mô</h3>
                            <div className="h-64">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={sizeStats.distribution}>
                                        <CartesianGrid strokeDasharray="3 3" />
                                        <XAxis dataKey="name" />
                                        <YAxis />
                                        <Tooltip />
                                        <Legend />
                                        <Bar dataKey="count" name="Số Lớp" fill="#8884d8" radius={[4, 4, 0, 0]} />
                                        <Bar dataKey="roomCount" name="Số Phòng" fill="#82ca9d" radius={[4, 4, 0, 0]} />
                                    </BarChart>
                                </ResponsiveContainer>
                            </div>
                            <p className="text-xs text-gray-500 mt-2 italic text-center">
                                *Cột tím cao hơn cột xanh ở bất kỳ nhóm nào báo hiệu nguy cơ thiếu phòng cục bộ.
                            </p>
                        </div>

                        {/* Equipment Demand */}
                        <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                            <h3 className="font-bold text-gray-700 mb-4">Nút thắt Thiết bị (Cung vs Cầu)</h3>
                                ...
                                <div className="h-64">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={equipmentStats} layout="vertical">
                                            ...
                                            <Tooltip />
                                            <Legend />
                                            <Bar dataKey="demand" name="Nhu cầu (Lớp)" fill="#f87171" barSize={20} />
                                            <Bar dataKey="supply" name="Cung ứng (Phòng)" fill="#60a5fa" barSize={20} />
                                        </BarChart>
                                    </ResponsiveContainer>
                                </div>
                            ) : (
                                <div className="h-64 flex items-center justify-center text-gray-400">
                                    Chưa có dữ liệu thiết bị trong Courses/Rooms
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className="p-6 border-t bg-gray-50 flex justify-end gap-3">
                    <button
                        onClick={onClose}
                        className="px-6 py-2 bg-white border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-100 font-medium"
                    >
                        Đóng & Chỉnh sửa
                    </button>
                    {/*<button*/}
                    {/*    className={`px-6 py-2 rounded-lg text-white font-bold flex items-center gap-2 ${*/}
                    {/*        loadStats.percentage > 100 || overloadedTeachers.length > 0*/}
                    {/*            ? 'bg-red-600 hover:bg-red-700'*/}
                    {/*            : 'bg-blue-600 hover:bg-blue-700'*/}
                    {/*    }`}*/}
                    {/*>*/}
                    {/*    {loadStats.percentage > 100 ? 'Vẫn cố gắng xếp lịch' : 'Tiến hành Xếp lịch'}*/}
                    {/*</button>*/}
                </div>
            </div>
        </div>
...
export default PreSchedulingDashboard;