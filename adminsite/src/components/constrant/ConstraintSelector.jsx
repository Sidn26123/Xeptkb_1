...
//         const isSelected = selectedConstraints.some(c => c.id === constraint.id);
//
//         if (isSelected) {
...
//
//     // Filter constraints
//     console.log('constraints in ConstraintSelector:', selectedConstraints);
...
//                             Đã chọn: <span className="font-semibold text-purple-600">{selectedConstraints.length}</span> / {filtered.length} ràng buộc
//                         </p>
//                     </div>
//
//                     {/* Controls */}
...
//                             >
//                                 Tất cả ({selectedConstraints.length})
//                             </Button>
//                             <Button
...
//                             >
//                                 Ràng buộc cứng ({selectedConstraints.filter(c => c.type === 'H').length})
//                             </Button>
//                             <Button
...
//                             >
//                                 Ràng buộc mềm ({selectedConstraints.filter(c => c.type === 'S').length})
//                             </Button>
//                         </div>
//
//                         {/* Selected Summary */}
...
//                                 Tổng số ràng buộc đã chọn: <span className="font-bold text-purple-600">{selectedConstraints.length}</span>
//                             </div>
...
//                                 <Button
//                                     variant="primary"
//                                     onClick={() => console.log('Selected:', selectedConstraints)}
//                                     disabled={selectedConstraints.length === 0}
//                                     className={selectedConstraints.length === 0 ? 'opacity-50 cursor-not-allowed' : ''}
//                                 >
//                                     Xác nhận ({selectedConstraints.length})
//                                 </Button>
//                             </div>
//                         </div>
//                     </div>
...
import {
    addConstraint,
    removeConstraint,
    setConstraints, // Hàm này set danh sách ĐÃ CHỌN vào store
    useSelectedConstraints
} from "../../stores/ScheduleDataStore.js";

export default function ConstraintSelector({ constraintsProps = [] }) {
    // constraintsProps: Danh sách TẤT CẢ ràng buộc từ API/Parent truyền vào

    const [search, setSearch] = useState('');
    const [selectedType, setSelectedType] = useState('all');

    // Lấy danh sách ĐÃ CHỌN từ store
    const selectedConstraints = useSelectedConstraints();

    // --- 1. SỬ DỤNG REF ĐỂ CHẶN LOOP ---
    // Biến này sẽ giữ giá trị true/false qua các lần render mà không bị reset
    const hasAutoSelectedRef = useRef(false);

    useEffect(() => {
        // Chỉ chạy nếu:
        // 1. Chưa từng auto select (check Ref)
        // 2. Có dữ liệu đầu vào (constraintsProps có length > 0)
        // 3. Store hiện tại đang rỗng (selectedConstraints.length === 0)
        if (
            !hasAutoSelectedRef.current &&
            constraintsProps &&
            constraintsProps.length > 0
        ) {
            if (selectedConstraints.length === 0) {
                const hardConstraints = constraintsProps.filter(c => c.type === 'H');

                // Update Store
                for (let constraint of hardConstraints) {
                    addConstraint(constraint);

                }
            }

            // Đánh dấu là ĐÃ CHẠY XONG -> Lần render sau sẽ không lọt vào if này nữa
            hasAutoSelectedRef.current = true;
        }
    }, [constraintsProps, selectedConstraints.length]);

    // --- 2. Xử lý Toggle ---
    const toggleSelection = (constraint) => {
        // Kiểm tra xem ràng buộc này đã có trong danh sách đã chọn chưa
        const isSelected = selectedConstraints.some(c => c.id === constraint.id);
    ...
    // Chọn tất cả (Dựa trên list đang hiển thị/filter)
    const selectAll = () => {
        // Gom các item đang hiển thị chưa được chọn và thêm vào store
        // Hoặc đơn giản là set toàn bộ filtered vào store (cẩn thận mất những item đang bị ẩn do filter)

        // Cách an toàn: Merge filtered vào selectedConstraints
        const newSelection = [...selectedConstraints];
        filtered.forEach(item => {
            if (!newSelection.some(s => s.id === item.id)) {
                newSelection.push(item);
            }
        });
        setConstraints(newSelection);
    };
    ...
    // --- 3. Filter danh sách hiển thị ---
    // QUAN TRỌNG: Filter trên constraintsProps (dữ liệu gốc) chứ không phải dữ liệu store
    const filtered = constraintsProps.filter(c => {
        const matchesSearch =
            (c.name || '').toLowerCase().includes(search.toLowerCase()) ||
            (c.code || '').toLowerCase().includes(search.toLowerCase()) ||
            (c.description || '').toLowerCase().includes(search.toLowerCase());
    ...
    const renderConstraintList = () => (
        <div className="space-y-2">
            {filtered.map(constraint => {
                // Kiểm tra trạng thái dựa trên store
                const isSelected = selectedConstraints.some(c => c.id === constraint.id);
                const isHard = constraint.type === 'H';
            ...
            <div className="max-w-4xl mx-auto">
                <div className="bg-white rounded-xl shadow-lg border border-gray-200">
                    ...
                    <div className="border-b border-gray-200 bg-gradient-to-r from-purple-50 to-pink-50 p-6 rounded-t-xl">
                        <h1 className="text-3xl font-bold text-gray-800 mb-2">Chọn Ràng buộc</h1>
                        <p className="text-gray-600">
                            Đã chọn: <span className="font-semibold text-purple-600">{selectedConstraints.length}</span> / {constraintsProps.length} ràng buộc
                        </p>
                    </div>
                    ...
                    <div className="p-6 space-y-4">
                        ...
                        <div className="flex gap-2 items-center">
                            ...
                            <Button
                                ...
                                variant={selectedType === 'all' ? 'primary' : 'outline'}
                            >
                                Tất cả ({constraintsProps.length})
                            </Button>
                            <Button
                                onClick={() => setSelectedType('H')}
                                size="sm"
                                variant={selectedType === 'H' ? 'danger' : 'outline'}
                            >
                                Ràng buộc cứng ({constraintsProps.filter(c => c.type === 'H').length})
                            </Button>
                            <Button
                                onClick={() => setSelectedType('S')}
                                size="sm"
                                variant={selectedType === 'S' ? 'success' : 'outline'}
                            >
                                Ràng buộc mềm ({constraintsProps.filter(c => c.type === 'S').length})
                            </Button>
                        </div>
                        ...
                        {selectedConstraints.length > 0 && (
                            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                                <h3 className="font-semibold text-purple-900 mb-2">Đang chọn ({selectedConstraints.length})</h3>
                                <div className="flex flex-wrap gap-2">
                                    {selectedConstraints.map(c => (
                                        <span
                                            key={c.id}
                    ...
                    <div className="border-t border-gray-200 p-6 bg-gray-50 rounded-b-xl">
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex gap-3">
                                {/* Nút Confirm có thể gọi 1 prop onConfirm nếu cần, hoặc chỉ log như cũ */}
                                <Button
                                    variant="primary"
                                    onClick={() => console.log('Final Selected Constraints:', selectedConstraints)}
                                    disabled={selectedConstraints.length === 0}
                                    className={selectedConstraints.length === 0 ? 'opacity-50 cursor-not-allowed' : ''}
                                >
                                    Xác nhận
