...
import {getAllRooms, createRoom, updateRoom, deleteRoom, getAllRoomsWithEquipment} from '../services/roomService.js';
...
export default function RoomManagement() {
    ...
    const fetchList = async () => {
        try {
            const data = await getAllRoomsWithEquipment();
    ...
    const handleEditOpen = async (r) => {
        ...
        try {
            ...
            const nextSelected = (existing || []).map(ex => ({
    ...
    const handleAddSubmit = async (e) => {
        ...
        try {
            ...
            if (selectedEquipments && selectedEquipments.length > 0) {
                await Promise.all(selectedEquipments.map(se => createRoomEquipment({
                    room_id: created.id,
                    equipment_id: Number(se.equipment_id),
                    quantity: Number(se.quantity || 1)
                })).map(p => p.catch(err => {
                    console.error('createRoomEquipment failed', err);
                })));
            }
    ...
    const handleEditSubmit = async (e) => {
        ...
        try {
            ...
            try {
                ...
                await Promise.all((existing || []).map(ex => deleteRoomEquipment(ex.id).catch(err => {
                    console.error('deleteRoomEquipment failed', err);
                })));
    ...
    const handleEquipmentCheckboxToggle = (equipmentId) => {
        setSelectedEquipments(prev => {
            ...
            const next = exists ? prev.filter(p => String(p.equipment_id) !== String(equipmentId)) : [...prev, {
    ...
    return (
        <>
                ...
                <div className="max-w-full overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <th className="px-5 py-3 font-medium text-gray-500 text-center text-theme-xs dark:text-gray-400">Sức
                                chứa
                            </th>
                            ...
                            <tr key={r.id}>
                                ...
                                <td className="px-4 py-3 text-center">
                                    <Button size="sm" variant="outline" className="mr-2"
                                            onClick={() => handleEditOpen(r)}>Sửa</Button>
                                    <Button size="sm" variant="danger" onClick={() => {
                                        if (window.confirm('Bạn có chắc muốn xóa phòng này?')) {
                                            deleteRoom(r.id).then(() => fetchList()).catch(e => console.error(e));
                                        }
                                    }}>Xóa</Button>
                                </td>
                            </tr>
            ...
            <Modal isOpen={isAddOpen} onClose={handleAddClose} className="max-w-2xl w-full mx-auto bg-white shadow-2xl">
                <div className="flex flex-col max-h-[90vh]">
                        ...
                        <form ref={addFormRef} className="space-y-4" onSubmit={handleAddSubmit}>
                            ...
                            <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                                <h3 className="text-lg font-semibold text-gray-700 mb-3 flex items-center">
                                    <span
                                        className="bg-blue-100 text-blue-700 rounded-full w-6 h-6 flex items-center justify-center mr-2 text-sm">1</span>
                                    Vị trí
                                </h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block mb-1 text-sm font-semibold text-gray-700">Cơ sở <span
                                            className="text-red-500">*</span></label>
                                        <select id="selectedCampus"
                                                className={`w-full rounded-lg px-3 py-2 border ${errors.selectedCampus ? 'border-red-500' : 'border-blue-500'} focus:ring-2 focus:ring-blue-500 focus:border-blue-500`}
                                                value={selectedCampus || ''} onChange={(e) => {
                                            const val = e.target.value;
                                            setSelectedCampus(val);
                                            // clear campus-specific error when user changes selection
                                            setErrors(prev => {
                                                const next = {...prev};
                                                delete next.selectedCampus;
                                                return next;
                                            });
                                        ...
                                        {errors.selectedCampus &&
                                            <p className="text-red-500 text-xs mt-1">{errors.selectedCampus}</p>}
                                    </div>
                                    <div>
                                        <label className="block mb-1 text-sm font-semibold text-gray-700">Tòa nhà <span
                                            className="text-red-500">*</span></label>
                                        <select id="buildings_id"
                                                className={`w-full rounded-lg px-3 py-2 border focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${errors.buildings_id ? 'border-red-500' : 'border-blue-500'}`}
                                                value={form.buildings_id || ''} onChange={(e) => {
                                            const next = {...form, buildings_id: e.target.value, floor_number: ''};
                                            setForm(next);
                                            // clear building-specific error
                                            setErrors(prev => {
                                                const n = {...prev};
                                                delete n.buildings_id;
                                                return n;
                                            });
                                        }}>
                                            <option value="">-- Chọn tòa --</option>
                                            {buildings.map(b => (<option key={b.id}
                                                                         value={b.id}>{b.name || b.id} {b.floor_count ? `(${b.floor_count} tầng)` : ''}</option>))}
                                        </select>
                                        {errors.buildings_id &&
                                            <p className="text-red-500 text-xs mt-1">{errors.buildings_id}</p>}
                                    </div>
                                    <div>
                                        ...
                                        {errors.floor_number &&
                                            <p className="text-red-500 text-xs mt-1">{errors.floor_number}</p>}
                                    </div>
                                    <div>
                                        <label className="block mb-1 text-sm font-semibold text-gray-700">Số phòng <span
                                            className="text-red-500">*</span></label>
                                        <input id="room_seq"
                                               className={`w-full rounded-lg px-3 py-2 border ${errors.room_seq ? 'border-red-500' : 'border-blue-500'} focus:ring-2 focus:ring-blue-500 focus:border-blue-500`}
                                               type="number" value={form.room_seq || ''}
                                               onChange={(e) => setFormField('room_seq', e.target.value)}
                                               placeholder="VD: 1"/>
                                        {errors.room_seq &&
                                            <p className="text-red-500 text-xs mt-1">{errors.room_seq}</p>}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                                <h3 className="text-lg font-semibold text-gray-700 mb-3 flex items-center">
                                    <span
                                        className="bg-blue-100 text-blue-700 rounded-full w-6 h-6 flex items-center justify-center mr-2 text-sm">2</span>
                                    Thông tin cơ bản
                                </h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block mb-1 text-sm font-semibold text-gray-700">Mã phòng <span
                                            className="text-red-500">*</span></label>
                                        <input id="code"
                                               className={`w-full rounded-lg px-3 py-2 border ${errors.code ? 'border-red-500' : 'border-blue-500'} focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-gray-100`}
                                               type="text" value={form.code} readOnly disabled placeholder="(Tự sinh)"/>
                                        {errors.code && <p className="text-red-500 text-xs mt-1">{errors.code}</p>}
                                    </div>
                                    <div>
                                        <label className="block mb-1 text-sm font-semibold text-gray-700">Tên
                                            phòng <span className="text-red-500">*</span></label>
                                        <input id="name"
                                               className={`w-full rounded-lg px-3 py-2 border ${errors.name ? 'border-red-500' : 'border-blue-500'} focus:ring-2 focus:ring-blue-500 focus:border-blue-500`}
                                    ...
                                    <div>
                                        <label className="block mb-1 text-sm font-semibold text-gray-700">Loại
                                            phòng <span className="text-red-500">*</span></label>
                                        <input id="type"
                                               className={`w-full rounded-lg px-3 py-2 border ${errors.type ? 'border-red-500' : 'border-blue-500'} focus:ring-2 focus:ring-blue-500 focus:border-blue-500`}
                                    ...
                                    <div>
                                        <label className="block mb-1 text-sm font-semibold text-gray-700">Trạng
                                            thái <span className="text-red-500">*</span></label>
                                        <select id="status"
                                                className={`w-full rounded-lg px-3 py-2 border ${errors.status ? 'border-red-500' : 'border-blue-500'} focus:ring-2 focus:ring-blue-500 focus:border-blue-500`}
                                                value={form.status}
                            ...
                            <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                                <h3 className="text-lg font-semibold text-gray-700 mb-3 flex items-center">
                                    <span
                                        className="bg-blue-100 text-blue-700 rounded-full w-6 h-6 flex items-center justify-center mr-2 text-sm">3</span>
                                    Sức chứa
                                </h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block mb-1 text-sm font-semibold text-gray-700">Sức chứa tối
                                            đa</label>
                                        <input id="capacity_max"
                                               className={`w-full rounded-lg px-3 py-2 border ${errors.capacity_max ? 'border-red-500' : 'border-blue-500'} focus:ring-2 focus:ring-blue-500 focus:border-blue-500`}
                                               type="number" value={form.capacity_max}
                                               onChange={(e) => setFormField('capacity_max', e.target.value)}
                                               placeholder="Số người tối đa"/>
                                        {errors.capacity_max &&
                                            <p className="text-red-500 text-xs mt-1">{errors.capacity_max}</p>}
                                    </div>
                                    <div>
                                        <label className="block mb-1 text-sm font-semibold text-gray-700">Sức chứa tối
                                            ưu</label>
                                        <input id="capacity_optimal"
                                               className={`w-full rounded-lg px-3 py-2 border ${errors.capacity_optimal ? 'border-red-500' : 'border-blue-500'} focus:ring-2 focus:ring-blue-500 focus:border-blue-500`}
                                               type="number" value={form.capacity_optimal}
                                               onChange={(e) => setFormField('capacity_optimal', e.target.value)}
                                               placeholder="Số người tối ưu"/>
                                        {errors.capacity_optimal &&
                                            <p className="text-red-500 text-xs mt-1">{errors.capacity_optimal}</p>}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                                <h3 className="text-lg font-semibold text-gray-700 mb-3 flex items-center">
                                    <span
                                        className="bg-blue-100 text-blue-700 rounded-full w-6 h-6 flex items-center justify-center mr-2 text-sm">4</span>
                                    Thiết bị
                                </h3>
                                {isLoadingEquipments ? (
                                    ...
                                    <>
                                        <div className="flex justify-between items-center mb-3">
                                            <span
                                                className="text-sm text-gray-600">Chọn thiết bị cho phòng ({selectedEquipments.length} đã chọn)</span>
                                            <div className="flex gap-2">
                                                <button type="button"
                                                        className="text-sm text-blue-600 hover:text-blue-800 font-medium"
                                                        onClick={selectAllEquipments}>Chọn tất cả
                                                </button>
                                                <span className="text-gray-400">|</span>
                                                <button type="button"
                                                        className="text-sm text-gray-600 hover:text-gray-800 font-medium"
                                                        onClick={clearAllEquipments}>Bỏ chọn
                                                </button>
                                            </div>
                                        </div>
                                        <div
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                                {equipments.map(eq => {
                                                    ...
                                                    return (
                                                        <div key={eq.id}
                                                             className="flex items-center gap-2 bg-white rounded p-2 hover:bg-blue-50 transition-colors">
                                                            <input
                                                                type="checkbox"
                                                                id={`equip-${eq.id}`}
                                                                checked={!!selected}
                                                            ...
                                                            <label htmlFor={`equip-${eq.id}`}
                                                                   className="flex-1 text-sm text-gray-700 cursor-pointer">{eq.name || eq.code || `#${eq.id}`}</label>
                                                            {selected && (
                                                                <input
                                                                    type="number"
                                                                    min="1"
                    ...
                    <div className="flex justify-end gap-3 p-6 bg-gray-50 border-t border-gray-200 rounded-b-xl">
                        <Button size="md" variant="outline" className="font-semibold px-6 py-2"
                                onClick={handleAddClose}>Hủy</Button>
                        <Button size="md" variant="primary"
                                className="bg-blue-600 hover:bg-blue-700 font-semibold px-6 py-2 disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            <Modal isOpen={isEditOpen} onClose={handleEditClose}
                   className="max-w-2xl w-full mx-auto bg-white shadow-2xl">
                <div className="flex flex-col max-h-[90vh]">
                    <div className="p-6 bg-linear-to-r from-blue-600 to-blue-700 rounded-t-xl">
                                                    ...
                                                    return (
                                                        <div key={eq.id}
                                                            ...
                                                            <input
                                                                ...
                                                                className="h-4 w-4 text-blue-600 border-blue-500 rounded focus:ring-blue-500"
                                                            />
                                                            <label htmlFor={`equip-edit-${eq.id}`}
                                                                   className="flex-1 text-sm text-gray-700 cursor-pointer">{eq.name || eq.code || `#${eq.id}`}</label>
                                                            {selected && (
                                                                <input
                                                                    type="number"
