        ...
        <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} className="max-w-lg w-full mx-auto bg-white shadow">
          <div className="p-6 sm:p-8 bg-white rounded-xl shadow border border-gray-200">
            <h2 className="text-2xl font-bold mb-4 text-center text-gray-800">
              {editSemester ? "Sửa học kỳ" : "Thêm học kỳ"}
            </h2>
            <form onSubmit={formInstance.handleSubmit(handleSubmit)} className="space-y-4">
              <div>
                <label className="text-sm text-gray-600">Mã học kỳ</label>
                <input {...formInstance.register("code")} className="border p-2 rounded w-full" placeholder="Nhập mã học kỳ" />
                {formInstance.formState.errors.code && <p className="text-red-500 text-sm">{formInstance.formState.errors.code.message}</p>}
              </div>

              <div>
                <label className="text-sm text-gray-600">Tên học kỳ</label>
                <input {...formInstance.register("name")} className="border p-2 rounded w-full" placeholder="Nhập tên học kỳ" />
                {formInstance.formState.errors.name && <p className="text-red-500 text-sm">{formInstance.formState.errors.name.message}</p>}
              </div>

              <div>
                <label className="text-sm text-gray-600">Năm học</label>
                <select {...formInstance.register("AcademicYearsid")} className="border p-2 rounded w-full">
                  <option value="">-- Chọn năm học --</option>
                  {academicYears.map((y) => (
                      <option key={y.id} value={y.id}>{y.year_code}</option>
                  ))}
                </select>
                {formInstance.formState.errors.AcademicYearsid && <p className="text-red-500 text-sm">{formInstance.formState.errors.AcademicYearsid.message}</p>}
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm text-gray-600">Ngày bắt đầu</label>
                  <input type="date" {...formInstance.register("start")} className="border p-2 rounded w-full" />
                  {formInstance.formState.errors.start && <p className="text-red-500 text-sm">{formInstance.formState.errors.start.message}</p>}
                </div>
                <div>
                  <label className="text-sm text-gray-600">Ngày kết thúc</label>
                  <input type="date" {...formInstance.register("end")} className="border p-2 rounded w-full" />
                  {formInstance.formState.errors.end && <p className="text-red-500 text-sm">{formInstance.formState.errors.end.message}</p>}
                </div>
              </div>

              <div>
                <label className="text-sm text-gray-600">Trạng thái</label>
                <select {...formInstance.register("status")} className="border p-2 rounded w-full">
                  <option value="">-- Chọn trạng thái --</option>
                  <option value="active">Đang diễn ra</option>
                  <option value="finished">Đã kết thúc</option>
                  <option value="upcoming">Sắp diễn ra</option>
                </select>
                {formInstance.formState.errors.status && <p className="text-red-500 text-sm">{formInstance.formState.errors.status.message}</p>}
              </div>
