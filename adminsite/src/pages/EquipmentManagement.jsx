...
export default function EquipmentManagement() {
    ...
    const handleAddOpen = () => {
        setForm({code: '', name: '', total: 0, description: ''});
        setErrors({});
        setIsAddOpen(true);
    };
    ...
    const handleEditOpen = (e) => {
        setEditEquipment(e);
        setForm({code: e.code || '', name: e.name || '', total: e.total || 0, description: e.description || ''});
        setErrors({});
        setIsEditOpen(true);
    };
    ...
    const handleOpenImport = () => {
        setImportErrors([]); // Xóa lỗi cũ của lần trước
        setIsImportOpen(true);
    };
    const handleImportData = async (data) => {
        setImportErrors([]);
        try {
            // Data ở đây đã được map key sang tiếng Anh nhờ component DataImportModal
            // Bạn có thể validate sơ bộ ở đây nếu muốn (ví dụ check null)

            await bulkImportEquipments(data);
            setIsImportOpen(false);
            fetchList()
            showSuccess('Nạp thành công' + (data.length || 0) + ' thiết bị');
        } catch (err) {
            ...
            }// Reuse logic hiển thị lỗi nếu backend trả về lỗi chi tiết cho từng dòng
            // const data = err?.response?.data;
            // if (data) applyServerValidation(data);
            // else showError(err?.message || String(err) || 'Lỗi không xác định');
        }
    };
                ...
                <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center gap-3">
                        <input value={search} onChange={(e) => setSearch(e.target.value)}
                               placeholder="Tìm theo tên hoặc mã thiết bị" className="border rounded px-3 py-2"/>
                        <Button size="sm" variant="outline" onClick={() => fetchList()}>Làm mới</Button>
                    </div>
                    <div className={"flex items-center gap-3"}>
                        <Button size="md" variant="outline" onClick={handleOpenImport}>
                            Import Excel
                        </Button>
                            ...
                            <tr key={s.id}>
                                ...
                                <td className="px-4 py-3 text-center">
                                    <Button size="sm" variant="outline" className="mr-2"
                                            onClick={() => handleEditOpen(s)}>Sửa</Button>
                ...
                <div className="p-6">
                    ...
                    <form onSubmit={handleCreate} className="space-y-4">
                        <div>
                            ...
                            <input id="code" className="w-full border rounded px-3 py-2" value={form.code}
                        ...
                        <div>
                            ...
                            <input id="name" className="w-full border rounded px-3 py-2" value={form.name}
                        ...
                        <div>
                            ...
                            <textarea id="description" className="w-full border rounded px-3 py-2" rows={3}
                        ...
                        <div>
                            ...
                            <input id="total" type="number" className="w-full border rounded px-3 py-2"
                                   value={form.total}
